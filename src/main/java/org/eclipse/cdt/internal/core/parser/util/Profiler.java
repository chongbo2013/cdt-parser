...
public class Profiler {
	...
	public static void printStats() {
		Profiler profiler = threadProfiler.get();
		if (profiler != null) {
			List<Entry<String, Timer>> list = new ArrayList<>(profiler.timers.entrySet());
			Comparator<Entry<String, Timer>> c = new Comparator<>() {
				@Override
			...
			if (!profiler.counters.isEmpty()) {
				List<Entry<String, int[]>> keyList = new ArrayList<>(profiler.counters.entrySet());
				Comparator<Entry<String, int[]>> c2 = new Comparator<>() {
					@Override
