...
public class MachOHelper64 {
	...
	private DyLib[] needed;
	private DyLib[] sonames;
	...
	public Symbol[] getExternalFunctions() throws IOException {
		...
		for (Symbol sym : dynsyms) {
			if ((sym.n_type_mask(Symbol.N_PEXT) || sym.n_type_mask(Symbol.N_EXT))
					&& sym.n_desc(Symbol.REFERENCE_FLAG_UNDEFINED_LAZY)) {
		...
		Symbol[] ret = v.toArray(new Symbol[0]);
		return ret;
	}
	...
	public Symbol[] getExternalObjects() throws IOException {
		...
		for (Symbol sym : dynsyms) {
			if ((sym.n_type_mask(Symbol.N_PEXT) || sym.n_type_mask(Symbol.N_EXT))
					&& sym.n_desc(Symbol.REFERENCE_FLAG_UNDEFINED_NON_LAZY)) {
				String name = sym.toString();
				if (name != null && name.trim().length() > 0)
					v.add(sym);
			}
	...
	public Symbol[] getUndefined() throws IOException {
		...
		for (Symbol dynsym : dynsyms) {
			if (dynsym.n_type(Symbol.N_UNDF))
				v.add(dynsym);
		}
	...
	public Symbol[] getLocalFunctions() throws IOException {
		...
		for (Symbol sym : dynsyms) {
			if ((!sym.n_type_mask(Symbol.N_PEXT) && !sym.n_type_mask(Symbol.N_EXT))
					&& sym.n_desc(Symbol.REFERENCE_FLAG_PRIVATE_UNDEFINED_LAZY)) {
				String name = sym.toString();
				if (name != null && name.trim().length() > 0)
					v.add(sym);
			}
	...
	public Symbol[] getLocalObjects() throws IOException {
		...
		for (Symbol sym : dynsyms) {
			if ((!sym.n_type_mask(Symbol.N_PEXT) && !sym.n_type_mask(Symbol.N_EXT))
					&& sym.n_desc(Symbol.REFERENCE_FLAG_PRIVATE_UNDEFINED_NON_LAZY)) {
				String name = sym.toString();
				if (name != null && name.trim().length() > 0)
					v.add(sym);
			}
	...
	public Symbol[] getCommonObjects() throws IOException {
		...
		for (int i = 0; i < dynsyms.length; i++) {
			...
			if (sym.n_type_mask(Symbol.N_EXT) && sym.n_type(Symbol.N_UNDF) && sym.n_value != 0) {
				v.add(symbols[i]);
			}
		}
	...
	public Sizes getSizes() throws IOException {
		...
		for (Section section : sections) {
			...
			if (section.flags(Section.SECTION_TYP) != Section.S_ZEROFILL) {
