...
public class MachO64 implements AutoCloseable {
		...
		protected MachOhdr() throws IOException {
			...
			if (magic == MH_CIGAM || magic == MH_CIGAM_64) {
				...
				while (numArchives-- > 0) {
					...
					if ((cpuType == MachOhdr.CPU_TYPE_I386
							&& (arch.equalsIgnoreCase("i386") || arch.equalsIgnoreCase("x86_64"))) || //$NON-NLS-1$ //$NON-NLS-2$
							(cpuType == MachOhdr.CPU_TYPE_POWERPC && arch.equalsIgnoreCase("ppc")) || //$NON-NLS-1$
							(cpuType == MachOhdr.CPU_TYPE_X86_64 && arch.equalsIgnoreCase("x86_64"))) { //$NON-NLS-1$
						if (cpuType == MachOhdr.CPU_TYPE_X86_64)
							b64 = true;
	...
	public Attribute getAttributes() throws IOException {
		...
		case MachOhdr.MH_OBJECT:
			attrib.type = Attribute.MACHO_TYPE_OBJ;
			break;
		case MachOhdr.MH_EXECUTE:
		case MachOhdr.MH_PRELOAD:
		case MachOhdr.MH_BUNDLE:
		case MachOhdr.MH_DYLINKER:
			attrib.type = Attribute.MACHO_TYPE_EXE;
			break;
		case MachOhdr.MH_CORE:
			attrib.type = Attribute.MACHO_TYPE_CORE;
			break;
		case MachOhdr.MH_DYLIB:
		case MachOhdr.MH_FVMLIB:
			attrib.type = Attribute.MACHO_TYPE_SHLIB;
			break;
		}

		switch (mhdr.cputype) {
		case MachOhdr.CPU_TYPE_X86_64:
			attrib.cpu = "x86_64"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_I386:
			attrib.cpu = "x86"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_POWERPC:
			attrib.cpu = "ppc"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_VAX:
			attrib.cpu = "vax"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_MC680x0:
			attrib.cpu = "m68k"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_MC98000:
			attrib.cpu = "98000"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_MC88000:
			attrib.cpu = "88000"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_HPPA:
			attrib.cpu = "hp"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_SPARC:
			attrib.cpu = "sparc"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_I860:
			attrib.cpu = "i860"; //$NON-NLS-1$
			break;
		case MachOhdr.CPU_TYPE_ANY:
		default:
			attrib.cpu = "any"; //$NON-NLS-1$
		}

		switch (mhdr.magic) {
		case MachOhdr.MH_CIGAM_64:
		case MachOhdr.MH_CIGAM:
			attrib.isle = true;
			break;
		case MachOhdr.MH_MAGIC_64:
		case MachOhdr.MH_MAGIC:
			attrib.isle = false;
			break;
		}
	...
	public static boolean isMachOHeader(byte[] bytes) {
		try {
			...
			return (magic == MachOhdr.MH_MAGIC || magic == MachOhdr.MH_CIGAM
					|| magic == MachOhdr.MH_UNIVERSAL || magic == MachOhdr.MH_CIGAM_64
					|| magic == MachOhdr.MH_MAGIC_64);
		} catch (IOException e) {
			return false;
		}
	}
