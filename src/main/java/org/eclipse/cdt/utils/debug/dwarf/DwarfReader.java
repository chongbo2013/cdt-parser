...
public class DwarfReader extends Dwarf implements ISymbolReader, ICompileOptionsFinder {
	...
	public void init(Elf exe) throws IOException {
		...
		Section[] sections = exe.getSections();
		...
		for (Section section : sections) {
			if (section.sh_type == Section.SHT_NOTE) {
				...
				if (data.remaining() > 12) {
					try {
						...
						String buildId = null;
						if (noteName.equals("GNU") && note_type == Section.NT_GNU_BUILD_ID) { //$NON-NLS-1$
		...
		if (!have_build_id) {
			...
			Section gnuDebugLink = exe.getSectionByName(DWARF_GNU_DEBUGLINK);
					...
					if (altInfoName.length() > 0) {
						...
						if (altFile.exists()) {
							try (Elf altInfo = new Elf(altFile.getCanonicalPath())) {
								Section[] altSections = altInfo.getSections();
