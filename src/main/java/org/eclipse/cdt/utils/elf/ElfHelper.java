...
public class ElfHelper implements AutoCloseable {
	...
	public Symbol[] getExternalFunctions() throws IOException {
		...
		for (int i = 0; i < dynsyms.length; i++) {
			if (dynsyms[i].st_bind() == Symbol.STB_GLOBAL && dynsyms[i].st_type() == Symbol.STT_FUNC) {
		...
		Symbol[] ret = v.toArray(new Symbol[v.size()]);
		return ret;
	}
	...
	public Symbol[] getExternalObjects() throws IOException {
		...
		for (int i = 0; i < dynsyms.length; i++) {
			if (dynsyms[i].st_bind() == Symbol.STB_GLOBAL && dynsyms[i].st_type() == Symbol.STT_OBJECT) {
				int idx = dynsyms[i].st_shndx;
				if (idx < Symbol.SHN_HIPROC && idx > Symbol.SHN_LOPROC) {
					String name = dynsyms[i].toString();
					if (name != null && name.trim().length() > 0)
	...
	public Symbol[] getUndefined() throws IOException {
		...
		for (int i = 0; i < dynsyms.length; i++) {
			if (dynsyms[i].st_shndx == Symbol.SHN_UNDEF)
				v.add(dynsyms[i]);
		}
	...
	public Symbol[] getLocalFunctions() throws IOException {
		...
		for (int i = 0; i < symbols.length; i++) {
			if (symbols[i].st_type() == Symbol.STT_FUNC) {
	...
	public Symbol[] getLocalObjects() throws IOException {
		...
		for (int i = 0; i < symbols.length; i++) {
			if (symbols[i].st_type() == Symbol.STT_OBJECT) {
				int idx = symbols[i].st_shndx;
				if (idx < Symbol.SHN_HIPROC && idx > Symbol.SHN_LOPROC) {
					String name = symbols[i].toString();
					if (name != null && name.trim().length() > 0)
	...
	public Symbol[] getCommonObjects() throws IOException {
		...
		for (int i = 0; i < symbols.length; i++) {
			if (symbols[i].st_bind() == Symbol.STB_GLOBAL && symbols[i].st_type() == Symbol.STT_OBJECT) {
				int idx = symbols[i].st_shndx;
	...
	public Dynamic[] getNeeded() throws IOException {
		...
		for (int i = 0; i < dynamics.length; i++) {
			if (dynamics[i].d_tag == Dynamic.DT_NEEDED)
				v.add(dynamics[i]);
		}
	...
	public String getSoname() throws IOException {
		...
		for (int i = 0; i < dynamics.length; i++) {
			if (dynamics[i].d_tag == Dynamic.DT_SONAME)
