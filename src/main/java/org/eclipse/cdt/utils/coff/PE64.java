...
public class PE64 implements AutoCloseable {
	...
	public PE64(String filename, long pos, boolean filter) throws IOException {
		try {
			...
			fileHeader = new FileHeader(rfile, rfile.getFilePointer());
			...
			if (fileHeader.f_opthdr > 0) {
				optionalHeader = new OptionalHeader(rfile, rfile.getFilePointer());
	...
	public static Attribute getAttribute(byte[] data) throws IOException {
		...
		if (idx < data.length) {
			...
			FileHeader filehdr = new FileHeader(bytes, true);
			...
			for (int i = 0; i < sections.length; i++) {
				...
				if (name.equals(".rdata")) { //$NON-NLS-1$
					...
					for (int j = 0; j < debugFormats; j++) {
						IMAGE_DEBUG_DIRECTORY dir = new IMAGE_DEBUG_DIRECTORY(accessFile, fileOffset);
